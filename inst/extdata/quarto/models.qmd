...
assessed for each repository in the [`aaa`  GitHub
organization](https://github.com/aaa). Details of individual metrics can be
seen on the [*Repository Maintenance* page](./repo.html).

## Aggregate Community Health

This page shows aggregate community health scores for all repositories within
the `aaa`  organization. Higher values describe repositories with greater
community health scores. Values for individual models are standardised across
all repositories to have mean values of zero and standard deviations of one.
These results are aggregated across thirteen distinct models, leading to
maximal aggregated values slightly exceeding that value.
