...
metric_desc <- rbind (
    c ("has_ci", "Lacking continuous integration:"),
    c ("test_coverage", "Low test coverage:")
)
m$name <- metric_desc [, 2] [match (m$name, metric_desc [, 1])]

metrics <- unique (m$name)
metrics_data <- lapply (metrics, function (i) {
    list (
        h = i,
        text = paste0 ("- ", m$package [which (m$name == i)])
    )
})
```
                ...
                lapply (metrics_data, function (i) {
                    htmltools::div (
                        ...
                        if (length (i$text) == 1L) {
                            ...
                            htmltools::tagList (lapply (i$text, function (txt) tags$p (txt)))
                        },
