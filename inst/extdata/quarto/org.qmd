...
pkg_deps <- do.call (rbind, unname (pkg_deps)) |>
    dplyr::filter (package %in% name)
pkg_deps_net <- lapply (
    split (pkg_deps, f = as.factor (pkg_deps$name)),
    function (i) i$package
)
ojs_define (pkg_deps_net_in = pkg_deps_net)
```


```{ojs}
pkg_node_names = Object.keys(pkg_deps_net_in);
pkg_nodes = Object.entries(pkg_deps_net_in).map(([name, array]) => ({
    name: name,
    size: array.length
}))
pkg_edges = Object.entries(pkg_deps_net_in).flatMap(([source, targets]) => {
    if (!Array.isArray(targets)) {
        return [];
    }
    return targets.map(target => ({ source, target }))
});
console.log("-------NODES: " + JSON.stringify(pkg_nodes));
console.log("-------EDGES: " + JSON.stringify(pkg_edges));
```
