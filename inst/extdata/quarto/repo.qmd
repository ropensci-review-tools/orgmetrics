...
into conceptual or thematic groups.
</div>`
```
...
lower or upper 10% of all repositories. Clicking on the following button will
toggle to display all values.
...
//
// This file has the text definitions for all metrics & models variables
...
model_repo_ht_mult = model_repo_len < 10 ? 40 : 20
```

<!--- break --->

```{ojs}
// ------ replace model variables with text from repometrics JSON file
textMap = new Map();
rm_models.forEach(item => {
    textMap.set(String(item.name), item.text);
});
model_repo.forEach(item => {
    if (textMap.has(item.name)) {
        item.name = textMap.get(item.name);
    }
})
```
<!--- break --->
...
Plot.plot({
    height: model_repo_ht_mult * metrics_repo_len,
    marginLeft: 60,
    marginRight: 160,
    marginTop: 50,
    marginBottom: 50,
