...
[aaa](https://github.com/aaa) GitHub organization, and some
...
```{r cards-with-time-series, echo = FALSE, message = TRUE}
lines <- paste0 (unname (repo_orgs), " from ", names (repo_orgs))
showcase_gh <- NULL
if (length (annual_gh_activity$year) > 0L &
    !any (is.na (annual_gh_activity$year))) {
    showcase_gh <- sparkline_plot_gh ()
}
bslib::page_fillable (
    bslib::layout_columns (
        bslib::value_box (
...
- Nodes show individual packages, with circles for each package scaled by total
numbers of dependent packages; that is, numbers of other packages which depend
on that package.
- Edges connect dependent packages, with thicknesses scaled by total numbers of
function calls by all dependent packages.

The diagram can be zoomed, moved, and dragged to dynamically explore how
packages inter-related. The initial "Repository" control highlights the
selected repository in red.


```{r network-data-from-r}
...
size <- 0L
if (nrow (pkg_deps) == 0L) size <- integer (0L)
nodes <- data.frame (
    id = unique (c (pkg_deps$package, pkg_deps$name)),
    size = size
) |>
    dplyr::arrange (tolower (id))
...
m <- data_metrics [[index1]] |>
    ...
    dplyr::arrange (by = desc (total))
if ("none" %in% names (m)) {
    m <- dplyr::select (m, -none)
}
ojs_define (metrics_table_in = m)
```

```{ojs ojs-metrics-grouped-table}
metricsGroupedTable = {
