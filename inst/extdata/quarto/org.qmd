...
cli::cli_inform ("reducing metrics from {length(data_metrics)} to {length(index)}")
data_metrics <- data_metrics [index]
# ----- END DATE FIX -----
...
```{r}
maintenance_ts <- readRDS ("results-maintenance-org.Rds") |>
    dplyr::group_by (date) |>
    dplyr::summarise (
...
# ----- START DATE FIX -----
# Date fixes as for metrics data above:
dates <- unique (maintenance_ts$date)
latest <- as.Date (max (dates))
date_seq <- repometrics:::get_end_date_seq (end_date = latest, num_years = 3)
index <- vapply (
    date_seq,
    function (i) which.min (abs (difftime (dates, i))),
...
# cli::cli_inform ("reducing metrics from {length(dates)} to {length(index)}")
maintenance_ts <- maintenance_ts |>
    dplyr::filter (date %in% dates [index])
# ----- END DATE FIX -----
...
```{r}
maintenance_repo <- readRDS ("results-maintenance-org.Rds") |>
...
```{r load-table-data}
data_extra_metrics <- readRDS ("results-data-ctb-absence.Rds") |>
    dplyr::filter (what == "absence") |>
    dplyr::group_by (repo) |>
    dplyr::summarise (ctb_absence = max (measure)) |>
    dplyr::full_join (readRDS ("results-data-issue-resp.Rds"), by = "repo") |>
    dplyr::full_join (readRDS ("results-data-issue-bugs.Rds"), by = "repo")
...
```{r load-data}
data_ctb_abs_ctb <- readRDS ("results-data-ctb-absence.Rds") |>
    dplyr::filter (what == "absence") |>
    dplyr::group_by (name) |>
    dplyr::summarise (measure = sum (measure))
