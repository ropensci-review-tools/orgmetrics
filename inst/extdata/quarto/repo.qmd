...
cran_data <- readRDS ("results-cran-checks.Rds")
not_cran <- gsub ("^.*\\/", "", attr (cran_data, "not_cran"))
attr (cran_data, "not_cran") <- NULL

gitlog_data <- readRDS ("results-gitlog.Rds")
```
...
ojs_define (gitlog_data_in = gitlog_data)
```
...
gitlogDataTranspose = {
    return transpose(gitlog_data_in).map(row => ({
        ...row,
        first_commit: new Date(row.first_commit)
    }));
}

repos_on_r_univ = FileAttachment("data_is_on_r_univ.json").json();
data_r_univ_jobs = FileAttachment("data_r_univ_jobs.json").json();
data_r_univ_builds = FileAttachment("data_r_univ_builds.json").json();
```
...
build_checks = r_univ_builds.filter(d => (d['status'] !== "success"));
...
htl.html`<div>This package is not on R-univers</div>`;
...
is_on_r_univ ? htl.html`<h4>Recent R-Universe Jobs</h4>` : undefined
