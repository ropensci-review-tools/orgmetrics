...
m <- readRDS ("results-models.Rds") |>
    ...
    dplyr::distinct () |>
    dplyr::mutate (value = round (100 * value)) |>
    tidyr::pivot_wider (names_from = name, values_from = value)
ojs_define (models_table_in = m)
```

```{ojs}
modelColumnTypes = FileAttachment('model-column-types.csv')
    .csv({typed: false})
    .then(rows => {
        return [...new Set(rows.map(row => row.group))]
    })
viewof columnTypesSelected = {
    const input =  Inputs.checkbox(
        modelColumnTypes,
        {
            value: modelColumnTypes,
            label: htl.html`<b>Categories:</b>`,
        }
    );
...
columnModels = FileAttachment('model-column-types.csv')
    .csv({typed: false})
    .then(rows => {
        return rows
            .filter(row => columnTypesSelected.includes(row.group))
            .map(row => row.model)
    })
```
...
```{ojs}
modelsTable = {
    return transpose(models_table_in).map(row => ({
        ...row,
    }));
}
// availableColumns = Object.keys(modelsTable[0]).filter(k => k !== 'package')
```

```{ojs}
filteredModelsTable = modelsTable.map(item => {
    const filteredTable = {package: item.package};
    for (const col of columnModels) {
        filteredTable[col] = item[col];
    }
...
Inputs.table(filteredModelsTable, {
    format: {
        dev_responsiveness: sparkbar(d3.max(modelsTable, d => d.dev_responsiveness)),
        proj_engagement: sparkbar(d3.max(modelsTable, d => d.proj_engagement)),
        proj_awareness: sparkbar(d3.max(modelsTable, d => d.proj_awareness)),
        community_activity: sparkbar(d3.max(modelsTable, d => d.community_activity)),
        oss_compliance: sparkbar(d3.max(modelsTable, d => d.oss_compliance)),
        viability_community: sparkbar(d3.max(modelsTable, d => d.viability_community)),
        viability_starter: sparkbar(d3.max(modelsTable, d => d.viability_starter)),
        viability_gov: sparkbar(d3.max(modelsTable, d => d.viability_gov)),
        viability_strategy: sparkbar(d3.max(modelsTable, d => d.viability_strategy)),
        collab_devel_index: sparkbar(d3.max(modelsTable, d => d.collab_devel_index)),
        comm_serv_support: sparkbar(d3.max(modelsTable, d => d.comm_serv_support)),
        starter_health: sparkbar(d3.max(modelsTable, d => d.starter_health)),
        comm_welcoming: sparkbar(d3.max(modelsTable, d => d.comm_welcoming)),
    },
    header: {
        dev_responsiveness: tooltip("Developer responsiveness", "Explain that"),
        proj_engagement: tooltip("Project engagement", "Some explainy stuff"),
        proj_awareness: "Project awareness",
        community_activity: "Community activity",
        oss_compliance: "OSS compliance",
        viability_community: "Viability: community",
        viability_starter: "Viability: starter",
        viability_gov: "Viability: governance",
        viability_strategy: "Viability: strategy",
        collab_devel_index: "Collaboration development index",
        comm_serv_support: "Community service and support",
        starter_health: "Starter health",
        comm_welcoming: "Community welcoming"
    },
})
```

<iframe class="airtable-embed"
    src="https://airtable.com/embed/app5rHNzxrTuoGB4u/shrOzCSDhxO7zlfD0?viewControls=on"
