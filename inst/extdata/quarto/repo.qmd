...
repos = Array.from(new Set(reposAll)).sort((a, b) =>
    a.localeCompare(b, undefined, { sensitivity: 'base' })
);

// -------- 5A. SEARCH PARAMETERS VS LOCALSTORAGE
searchParams = new URLSearchParams(window.location.search);
initialRepoSet = function(searchParams, repos) {
    var initialRepo = searchParams.get('repo');
    if (initialRepo) {
        localStorage.setItem("orgmetricsRepo", initialRepo);
    } else {
        initialRepo = localStorage.getItem("orgmetricsRepo");
    }

    if (!initialRepo) {
        initialRepo = repos [Math.floor(Math.random() * repos.length)];
    }

    return initialRepo;
}

repoSet = initialRepoSet(searchParams, repos);
...
searchParamsRepo = updateSearchParams(searchParams, 'repo', repo.toString());
url = window.location.pathname;
hash = window.location.hash;
url_new = `${url}?${searchParamsRepo.toString()}${hash}`;
...
similar_pkgs = matched_names.map(i => {
    ...
    return htl.html`<li><a href="../repo.html?repo=${i}" onclick="localStorage.setItem('orgmetricsRepo', '${i}')">${i}</a></li>`
});
```
...
<li>
<a href='../repo.html?repo=${d}' onclick="localStorage.setItem('orgmetricsRepo', '${d}')">${d}</a>
</li>`);
...
higher values are always better than lower values. Note that metrics and models
are only assessed over the previous year. Respositories that have been inactive
during this time will thus have many scores of zero.
</div>`
```
