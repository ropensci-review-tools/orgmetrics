...
```{ojs}
// ------ load model data from R input and replace variables with text
// ------ from repometrics JSON file.
...
model_repoAll_varnames = models_all.filter(function(mod) {
    return mod.package === repo && mod.name !== "final"
})

model_repoAll = model_repoAll_varnames.map(item => {
