...
nodes = [
    { id: maintainer, group: "maintainer", num_calls: 1 },
    ...pkgs_expanded.map(item => ({
        id: item,
        group: these_pkgs.includes(item) ? "core" : "package",
        num_calls: 2
    })),
    ...these_cos.map(item => ({ id: item, group: "maintainer", num_calls: 1 }))
];
...
links = [
    ...these_pkgs.map(item => ({
        source: maintainer, target: item, value: 2, type: "direct"
    })),
    ...these_co_pkgs_flat.map(item => ({
        source: item.source, target: item.target, value: 1, type: "indirect"
    }))
];
```
